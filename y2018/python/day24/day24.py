...
from battle import Group, Battle
...
with open('day24.txt') as read:
    ...
    for l in read.readlines():
        ...
        if l == imm_marker:
            ...
            group = Group(
                good, int(sz), int(hp), stats[0], stats[1], int(atk), atk_type, int(init)
            )
            units.append(group)
...
"""
ok look for some reason
my code runs FOREVER if boost_amt is 76
so uh yeah just do a binary search or whatever to figure it out
"""
boost_amt = 77
for u in units:
    if u.good:
        u.atk += boost_amt
print(f"min units given a boost & a reindeer win: {Battle(units).get_res()[1]}")
