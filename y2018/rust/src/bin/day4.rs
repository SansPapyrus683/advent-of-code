...
fn main() {
    ...
    let mut events: Vec<(NaiveDateTime, Event)> = fs::read_to_string("../input/day4.txt").unwrap().lines().map(|e| {
        ...
        return (
            time,
            if what == "wakes up" { Event::Wake } else if what == "falls asleep" { Event::Sleep } else {
    ...
    for (t, e) in &events {
        match e {
            ...
            Event::Wake => {
                ...
                while curr_at < *t {
                    minutes_slept.get_mut(&curr_guard).unwrap()
            ...
            Event::Begin(id) => { curr_guard = *id; }
        }
    }

    let guard_prod = |(g, m): (&u32, &HashMap<u32, u32>)| g * m.iter()
        .max_by_key(|i| i.1)
        .unwrap().0;

    let strat1 = minutes_slept.iter()
