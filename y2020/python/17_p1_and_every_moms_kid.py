...
plane = [[int(i == "#") for i in line.rstrip()] for line in sys.stdin]
assert len(plane) == len(plane[0]), "can you just give a square?"
...
for _ in range(SIM_AMT):
    ...
    expanded[1 : new_z - 1, 1 : x_and_y - 1, 1 : x_and_y - 1] = (
        arr  # copy the array into the middle of the new one
    )
    arr = expanded
