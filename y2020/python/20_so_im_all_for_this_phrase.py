...
tiles = {}
for raw_tile in sys.stdin.read().split("\n\n"):
...
for t_id, tile in tiles.items():
    edges[t_id] = [
        [tile[0], tile[-1]],
        ["".join(r[0] for r in tile), "".join(r[-1] for r in tile)],
    ]
    ...
    try:
        ...
        for r in range(side_len):
            for c in range(side_len):
                ...
                filled_alr = [
                    p
                    for p in neighbors(side_len, r, c)
                    if actual_pic[p[0]][p[1]] is not None
                ].pop()
        ...
        IndexError
    ):  # well, that starting orientation wasn't valid, let's try another
        pass
