...
def valid_field(field: str, value: str) -> bool:
    if field == "byr":
        ...
        return (
            value[0] == "#"
            and len(value) == 7
            and all(v.isdigit or v.isalpha() for v in value[1:])
        )
    elif field == "ecl":
...
for line in sys.stdin:
    line = line.rstrip()
    if not line:
        passports.append(curr_string.strip().split())
        curr_string = ""
    else:
        curr_string += line + " "
passports.append(
    curr_string.strip().split()
)  # there's still a string left in day1.txt, let's append that
