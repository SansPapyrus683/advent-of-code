...
boarding_passes = [
    [p[:ROW_POW], p[ROW_POW : ROW_POW + COL_POW].rstrip()] for p in sys.stdin
]
...
for b in boarding_passes:
    ...
    for i in b[0]:  # doing all that binary searching for usaco did help, neat
        if i == "B":
            ...
            raise ValueError("wait are you sure your day1.txt's valid?")
...
for i in range(
    min(seat_ids) + 1, max(seat_ids)
):  # get the lower and upper bound for all the seats
    if i not in seat_ids:
        print(
            f"wait aren't online boarding passes a thing? couldn't we've just looked there: {i}"
        )
        break
