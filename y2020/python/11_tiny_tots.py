...
def p1_neighbors(row: int, col: int, r_max: int, c_max: int) -> list[tuple[int, int]]:
    return [
        p
        for p in [
            (row + 1, col),
...
@lru_cache(None)
def p2_neighbors(row: int, col: int, r_max: int, c_max: int, seats: tuple[str]):
    neighbor_funcs = [
        lambda x, y: (x + 1, y),
        lambda x, y: (x - 1, y),
        lambda x, y: (x, y - 1),
...
seats = [line.rstrip() for line in sys.stdin]
...
while True:
    ...
    for r in range(max_r):
        ...
        for c in range(max_c):
            ...
            if seats[r][c] == "L" and all(seats[i][j] != "#" for i, j in neighbors):
                new_row += "#"
            elif (
                seats[r][c] == "#"
                and sum(seats[i][j] == "#" for i, j in neighbors) >= 4
            ):
                new_row += "L"
            else:
                new_row += seats[r][c]
...
occ_num = sum(r.count("#") for r in seats)
print(f"this ran slower than expected, oof: {occ_num}")
    ...
    for r in range(max_r):
        ...
        for c in range(max_c):
            ...
            if seats[r][c] == "L" and all(seats[i][j] != "#" for i, j in neighbors):
                ...
                seats[r][c] == "#"
                and sum(seats[i][j] == "#" for i, j in neighbors) >= 5
            ):
                new_row += "L"
            else:
                new_row += seats[r][c]
...
print(
    "don't you hate it when you have two segments "
