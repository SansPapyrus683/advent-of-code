...
def line_up(edge1: [[str]], edge2: [[str]]) -> int:
    """
...
def vertical_neighbors(side: int, r: int, c: int) -> [[int]]:
    return [p for p in [[r + 1, c], [r - 1, c]] if 0 <= p[0] < side]


def horizontal_neighbors(side: int, r: int, c: int) -> [[int]]:
    return [p for p in [[r, c + 1], [r, c - 1]] if 0 <= p[1] < side]


def neighbors(side: int, r: int, c: int) -> [[int]]:
    return vertical_neighbors(side, r, c) + horizontal_neighbors(side, r, c)


def orientations(grid: [str]) -> [[str]]:
    """
