...
def path_num_p1(adj: dict[str, list[str]]) -> int:
    all_paths = set()

    def get_all_paths(at: str, path: list[str]):
        for n in adj[at]:
            if n == END:
...
def path_num_p2(adj: dict[str, list[str]]) -> int:
    all_paths = set()

    def get_all_paths(at: str, path: list[str], vis_small_twice: bool):
        for n in adj[at]:
            if n == END:
                all_paths.add(tuple(path + [END]))
            elif n.isupper():
