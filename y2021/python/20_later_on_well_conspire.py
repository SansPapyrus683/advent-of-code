...
input()
raw_img = [l.strip() for l in sys.stdin]

algo = np.array([i == LIT for i in algo], dtype=bool)
img = np.array([[i == LIT for i in r] for r in raw_img], dtype=bool)
...
for i in range(max(P1_AMT, P2_AMT)):
    ...
    expanded_img = np.zeros((len(img) + 4, len(img[0]) + 4), dtype=bool)
    expanded_img.fill(border)
    expanded_img[2:len(expanded_img) - 2, 2:len(expanded_img[0]) - 2] = img

    new_img = np.zeros((len(img) + 4, len(img[0]) + 4), dtype=bool)
