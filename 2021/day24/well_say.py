...
def get_valid(divs: [int], val1: [int], val2: [int], most: bool = True) -> int:
    ...
    def solve(z: int, ind_at: int, curr: [int]):
        nonlocal valid
        if ind_at == len(divs):
...
instructions = [i.strip() for i in open("no_lol.txt").readlines()]
...
for s in segments:
    s = "\n".join(s)
    assert re.match(TO_MATCH, s), "the input should be like that regex str"
