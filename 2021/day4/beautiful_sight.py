...
def score(board_nums: [[int]], have: [[bool]], last_num: int) -> int:
    total = 0
    for r in range(LEN):
        for c in range(LEN):
...
with open("we_happy_2nite.txt") as read:
    nums = [int(i) for i in read.readline().split(',')]
    boards = read.readlines()
    new_board = []
...
selected = [
    [[False for _ in range(LEN)] for _ in range(LEN)]
